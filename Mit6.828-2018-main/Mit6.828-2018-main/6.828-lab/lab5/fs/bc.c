...
va_is_dirty(void *va)
{	
	// Page table的属性中,有一个属性表示Dirty,
	// 如果dirty,返回1,如果不是,返回0
...
bc_pgfault(struct UTrapframe *utf)
{
	...
	//
	// LAB 5: you code here:
	void *alinged_addr = ROUNDDOWN(addr,PGSIZE);
	//首先先给引起fault的页面申请一个page
	...
	*/
	...
	// block from disk
	if ((r = sys_page_map(0, alinged_addr, 0, alinged_addr, uvpt[PGNUM(addr)] & PTE_SYSCALL)) < 0)
